...
public class SudokuController implements I_InputHandler {
    ...
    public void handleKeyInput(int row, int col, KeyEvent keyEvent) {
        ...
        if (key.matches("[1-6]")) {
            ...
            showAlert(Alert.AlertType.WARNING, "Invalid Entry",
                    "Please enter only numbers from 1 to 6");
            keyEvent.consume();
            return;
        }
    ...
    public void updateView() {
        ...
        messageLabel.setText("Relax, think, and enjoy solving it");
    }
    ...
    private void handleCheckBoard() {
        if (model.isBoardSolved()) {
            showStyledAlert(Alert.AlertType.INFORMATION, "🎉 ¡You did it!",
                    "¡You have solved the Sudoku correctly!");
        } else {
            showStyledAlert(Alert.AlertType.ERROR, "❌ Board Verification",
                    "The board is still unresolved or contains errors (highlighted in red).");
        }
        updateView();
    }
    ...
    private void handleHelpOption() {
        ...
        if (hint != null) {
            ...
            messageLabel.setText(String.format("💡 Hint provided: %d placed at (%d, %d).",
            ...
            showStyledAlert(Alert.AlertType.INFORMATION, "💡 Hint",
                    "The board is now full, or there are no valid moves possible.");
        }
    }
